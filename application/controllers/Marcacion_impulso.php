...
class Marcacion_impulso extends CI_Controller
{
	...
	public function registrar_actividad()
	{
		...
		$imagenEnBase64 = $this->input->post('Img_inicio_64');
		$imagenBinaria = base64_decode($imagenEnBase64);

		$name= $fechafinalfinal . "_" . $this->session->userdata('Id_u_sdv') . "_" . $this->input->post('txtclientes') . "_" . "INICIADA.jpg" ;

		$rutaImagenSalida = "Uploads/Imagenes/Marcaciones_impulso/".$name;

		
		file_put_contents($rutaImagenSalida, $imagenBinaria);
		...
		// if (!$this->upload->do_upload($imagenBinaria)) {
		...
		$data = array(
			...
			'Img_inicio' => base_url() . "Uploads/Imagenes/Marcaciones_impulso/" . $name,
			'Latitud_fin' => 0,
			'Longitud_fin' => 0,
			'Fecha_fin' => 0,
			'Img_fin' => 0,
	...
	public function finalizar_actividad()
	{
		...
		$imagenEnBase64 = $this->input->post('Img_fin_64');
		$imagenBinaria = base64_decode($imagenEnBase64);

		$name= $fechafinalfinal . "_" . $this->session->userdata('Id_u_sdv') . "_" . $this->input->post('txtclientesfin') . "_" . "FINALIZADA.jpg" ;
		...
		$data1 = array(
			...
			'Img_fin' => base_url() . "Uploads/Imagenes/Marcaciones_impulso/" . $name,
